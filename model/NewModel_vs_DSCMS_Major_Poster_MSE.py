...
all_MSE = MSELoss(reduction='none')
...
fig.savefig("FinalReportFigures/NewModel_vs_DSCMS_Major_Poster_MSE/Figure.png", dpi=800, bbox_inches='tight')
with open("FinalReportFigures/NewModel_vs_DSCMS_Major_Poster_MSE/Stats.txt", "w") as f:
    f.write("MSE\n")
    for i in range(2):
...
Fig, ax = plt.subplots()
print(torch.permute(out2, (2,0,1)).shape, label_to_tensor(label[...,1,:]).shape)
lossImg1 = all_MSE(torch.permute(out2, (2,0,1)), label_to_tensor(label[...,1,:]))
print(lossImg1.shape)
minLoss = np.min(np.sum(torch.permute(lossImg1, (1,2,0)).data.numpy(), axis=2))
maxLoss = np.max(np.sum(torch.permute(lossImg1, (1,2,0)).data.numpy(), axis=2))
pcm = ax.imshow(np.sum(torch.permute(lossImg1, (1,2,0)).data.numpy(), axis=2), cmap=lossCmap, vmin=minLoss, vmax=maxLoss)
ax.axis("off")
Fig.colorbar(pcm, ax=ax)
#ax.set_title(Titles[i])
Fig.savefig(f"FinalReportFigures/NewModel_vs_DSCMS_Major_Poster_MSE/{Titles[0]}2.png", dpi=600, bbox_inches='tight')



Fig, ax = plt.subplots()
lossImg2 = all_MSE(torch.permute(out, (2,0,1)), label_to_tensor(label[...,1,:]))
pcm = ax.imshow(np.sum(torch.permute(lossImg2, (1,2,0)).data.numpy(), axis=2), cmap=lossCmap, vmin=minLoss, vmax=maxLoss)
ax.axis("off")
Fig.colorbar(pcm, ax=ax)
#ax.set_title(Titles[i])
Fig.savefig(f"FinalReportFigures/NewModel_vs_DSCMS_Major_Poster_MSE/{Titles[1]}2.png", dpi=600, bbox_inches='tight')

Fig, axes = plt.subplots(1,2, figsize=(7,5))
pcm = axes[0].imshow(np.sum(torch.permute(lossImg1, (1,2,0)).data.numpy(), axis=2), cmap=lossCmap, vmin=minLoss, vmax=maxLoss)
axes[0].axis("off")
axes[0].set_title(Titles[0])
axes[1].imshow(np.sum(torch.permute(lossImg2, (1,2,0)).data.numpy(), axis=2), cmap=lossCmap, vmin=minLoss, vmax=maxLoss)
axes[1].axis("off")
axes[1].set_title(Titles[1])
Fig.colorbar(pcm, ax=axes[:])
#ax.set_title(Titles[i])
Fig.savefig(f"FinalReportFigures/NewModel_vs_DSCMS_Major_Poster_MSE/MSE_Diff_Output.png", dpi=600, bbox_inches='tight')

...
Checkpoints = [join(MODEL_PATH, "Friday-01-12-2023/", CHECKPOINT_PATH),
               join(MODEL_PATH, "Friday-01-12-2023-2/", CHECKPOINT_PATH), 
               join(MODEL_PATH, "Friday-01-12-2023-3/", CHECKPOINT_PATH),
               join(MODEL_PATH, "Friday-01-12-2023-4/", CHECKPOINT_PATH),
               join(MODEL_PATH, "Friday-01-12-2023-5/", CHECKPOINT_PATH)]
Titles = [r"$\gamma = 0.9$", r"$\gamma = 0.99$", r"$\gamma = 0.999$", r"$\gamma = 0.9999$", r"$\gamma = 0.99999$"]
...
fig, axes = plt.subplots(1,7, figsize=(11,5))
...
axes[6].imshow(np.sum(label[...,1,:]**2, axis=2), vmin=min_Val, vmax=max_Val)
axes[6].axis("off")
axes[6].set_title("DNS (reference)")
fig.savefig("FinalReportFigures/NewModel_DifferentMixedLosses/Figure.png", dpi=400, bbox_inches='tight')
with open("FinalReportFigures/NewModel_DifferentMixedLosses/Stats.txt", "w") as f:
    f.write("MSE\n")
    for i in range(5):

...
def test(model):
    ...
    with torch.no_grad():
        ...
        for index, batch in enumerate(testing_data_loader):
            ...
            mae = criterion_mae(prediction, target)
            ...
            psnr = 10 * log10(torch.max(target) / batch_mse)
            avg_psnr += psnr
    ...
    avg_losses = np.array(full_losses)/len(testing_data_loader)
...
CHECKPOINT_PATH = "../Logs_from_the_cloud/Wednesday-06-12-2023-2/model_checkpoints/model_epoch_30.pth"
SAVE_PATH = "test_logs/"
...
full_loss1, avg_loss1, list_loss1 = test(model1)
model1_end = perf_counter()
print(f"Time taken for model 1 = {model1_end-model1_start}")

M1_Path = join(SAVE_PATH, "DSMSC-B128-E120-L001-MSELoss")
...
full_loss2, avg_loss2, list_loss2 = test(model2)
model2_end = perf_counter()
print(f"Time taken for model 2 = {model2_end-model2_start}")

M2_Path = join(SAVE_PATH, "Bicubic-NoBlurData")

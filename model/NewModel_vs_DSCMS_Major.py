...
Checkpoints = [join(MODEL_PATH, "Wednesday-06-12-2023/", CHECKPOINT_PATH),
               join(MODEL_PATH, "Wednesday-06-12-2023-2/", CHECKPOINT_PATH)]
Titles = ["Bicubic\nInterpolation", "Modified\nSimple SR", "DSC\MS with\nPixle Shuffle"]
...
for i in range(2):
    ...
    MSEs.append(model_mse.item())
    PSNRs.append(20 * log10(np.max(label[...,1,:]) / np.sqrt(MSEs[i+1])))
    MAEs.append(model_mae.item())
    DIVs.append(model_div.item())
...
fig.savefig("FinalReportFigures/NewModel_vs_DSCMS_Major/Figure.png", dpi=400, bbox_inches='tight')
with open("FinalReportFigures/NewModel_vs_DSCMS_Major/Stats.txt", "w") as f:
    f.write("MSE\n")
    for i in range(3):
